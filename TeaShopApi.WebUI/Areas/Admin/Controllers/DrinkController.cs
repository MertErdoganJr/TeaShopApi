...
namespace TeaShopApi.WebUI.Areas.Admin.Controllers
{
    ...
    public class DrinkController : Controller
    {
        ...
        public async Task<IActionResult> CreateDrink(CreateDrinkDto createDrinkDto)
        {
            ...
            var respongseMessage = await client.PostAsync("https://localhost:7069/api/Drinks", content);
            if (respongseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7069/api/Drinks?id=" + id);
            if(responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
